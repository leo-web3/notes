...
- Client （客户端）
    从Provider(提供者)接收远程过程调用 (RPC) 请求并返回其结果的端点。
- Wallet (钱包)
    管理私钥、执行签名操作并充当Provider(提供者)和客户端之间的中间件的最终用户应用程序。
- Remote Procedure Call (RPC) (远程过程调用 (RPC))
    远程过程调用 (RPC) 是提交给Provider(提供者)的任何请求，要求Provider(提供者)、其电子钱包或其客户端处理某些过程。
...
>要为 `RPC` 请求提供服务，Provider(提供者)**必须**成功地将请求提交到远程位置，并收到响应。换句话说，如果 `Provider` 无法与其 `Client` 通信，例如由于网络问题，`Provider` 将断开连接。

## 应用程序接口 (API)
>`Provider API` 是使用 `TypeScript` 指定的。作者鼓励实现者声明他们自己的类型和接口，使用本节中的类型和接口作为基础。
>有关面向消费者的 API 文档，请参阅[附录 I](#附录-i面向消费者的-api-文档)

Provider(提供者)**必须**实现并公开本节中定义的 API。所有 API 实体都**必须**遵守本节中定义的类型和接口。
...
```
Provider(提供者)必须通过 `RequestArguments.method` 的值来识别请求的 RPC 方法。

如果请求的 `RPC` 方法采用任何参数，Provider(提供者)**必须**接受它们作为`RequestArguments.params`的值。
...
- Provider(提供者)遇到错误或由于任何原因无法处理请求。
...
- `Provider`(提供者)已断开连接。
    + 如果因为这个原因拒绝，`Promise` 拒绝错误`code` **必须**是`4900`。
- `RPC` 请求针对特定的链，Provider 不连接到该链，但连接到至少一个其他链。
    + 如果因为这个原因拒绝，`Promise` 拒绝错误`code` **必须**是`4901`。

有关“已连接”和“已断开连接”的定义，请参阅[连接性](#连接性-connectivity)部分。

### 支持的 RPC 方法
“支持的 `RPC` 方法”是可以通过Provider(提供者)调用的任何 `RPC` 方法。

所有受支持的 `RPC` 方法**必须**由唯一的字符串标识。

Provider(提供者)可以支持实现其目的所需的任何 `RPC` 方法，无论是标准化的还是其他方式。

如果不支持在最终 EIP 中定义的 `RPC` 方法，它应该`4200`根据下面的[Provider(提供者)错误](#Provider(提供者)错误-provider-errors)部分的错误被拒绝，或者根据 `RPC` 方法的规范适当的错误。
...
#### Provider(提供者)错误 (Provider Errors)

| Status code | Name                  | Description                                                              |
| ----------- | --------------------- | ------------------------------------------------------------------------ |
| 4001        | User Rejected Request | 用户拒绝了请求。                                           |
| 4100        | Unauthorized          | 请求的方法和/或帐户未被用户授权。 |
| 4200        | Unsupported Method    | Provider(提供者)不支持请求的方法。                      |
| 4900        | Disconnected          | Provider(提供者) (Provider) 与所有链断开连接。                            |
| 4901        | Chain Disconnected    | Provider(提供者) (Provider) 未连接到请求的链。                    |

>`4900`旨在指示Provider(提供者)与所有链断开连接，而`4901`旨在指示`Provider`(提供者)仅与特定链断开连接。换句话说，`4901`意味着`Provider`(提供者)连接到其他链，而不是请求的链。
...
>为了满足这些要求，Provider(提供者)实施者应该考虑简单地扩展 Node.js`EventEmitter`类并将其捆绑到目标环境中。
...
如果Provider(提供者)支持以太坊 `RPC` 订阅，例如 `eth_subscribe`，Provider(提供者)**必须**在收到订阅通知时发出`message(消息)`事件。
如果Provider(提供者)从`eth_subscribe`订阅中接受到订阅信息，Provider(提供者)**必须**使用以下形式的 `ProviderMessage` 对象发出消息事件：
```typescript copy
...
根据 `eth_chainId` 以太坊 `RPC` `方法，chainId` **必须**将连接链的整数 `ID` 指定为十六进制字符串。

#### 断开连接 (disconnect)

有关 `disconnect`(断开连接) 的定义，请参阅[`connectivity`(连接性)](#连接性-connectivity)部分。

如果`Provider`(提供者)与所有链`disconnect`(断开连接)，`Provider`(提供者)**必须**根据[`RPC` 错误](#rpc-错误)部分中定义的接口发出名为 `disconnect` 的事件，其值为 `error: ProviderRpcError`。 错误代码属性的值必须跟在 `CloseEvent` 的[状态代码](https://developer.mozilla.org/en-US/docs/Web/API/CloseEvent#Status_codes)之后。

### 切换链 (chainChanged)

如果Provider(提供者)连接到的链发生变化，Provider(提供者)**必须**发出名为 `chainChanged` 的事件，其值为 `chainId: string`，根据 `eth_chainId` 以太坊 `RPC` 方法，将新链的整数 `ID` 指定为十六进制字符串。

### 切换钱包帐号 (accountsChanged)

如果Provider(提供者)可用的账户发生变化，Provider(提供者)**必须**发出名为 `accountsChanged` 的事件，其值为 `accounts: string[]`，其中包含每个 `eth_accounts` 以太坊 `RPC` 方法的账户地址。

当 `eth_accounts` 的返回值改变时，“accounts available to the Provider(提供者可用的账户)”也会改变。

## 基本原理 (Rationale)

Provider(提供者)的目的是为消费者提供对以太坊的访问。通常，Provider(提供者)**必须**使以太坊 Web 应用程序能够做两件事：
...
- 更改Provider(提供者)发出 `RPC` 请求的能力
    + `connect`
    + `disconnect`
- 任何重要应用程序**必须**处理的常见客户端和/或钱包状态更改
    + `chainChanged`
    + `accountsChanged`
...
- [ethProvider(提供者)](https://www.npmjs.com/package/eth-provider)
...
它仅处理 `RPC` 消息并发出事件。因此，账户安全和用户隐私需要在Provider(提供者)和它的以太坊客户端之间的中间件中实现。
...
因此，最好将 `Provider` 对象视为由对手控制。Provider(提供者)实施者通过确保以下内容来保护用户、钱包和客户，这一点至关重要：

- Provider(提供者)不包含任何私人用户数据。
...
`eth_chainId`由于所有以太坊操作都针对特定链，因此根据以太坊 `RPC` 方法（请参阅[EIP-695](/web3/eip/eip695)），Provider(提供者)准确反映客户端配置的链非常重要。

这包括确保`eth_chainId`具有正确的返回值，并且`chainChanged`只要该值发生变化就会发出事件。

### 用户帐户公开和帐户更改 (User Account Exposure and Account Changes)

许多以太坊写入操作（例如`eth_sendTransaction`）需要指定用户帐户。Provider(提供者)消费者通过 `RPC` 方法访问这些帐户`eth_accounts`，并监听`accountsChanged`事件。
...
相反，Provider(提供者)应该支持 `RPC` 方法来明确请求帐户访问，例如`eth_requestAccounts`（参考[EIP-1102](/web3/eip/eip1102)）或`wallet_requestPermissions`（参考[EIP-2255](/web3/eip/eip2255)）。
...
```
`chainId`该事件根据`Ethereum RPC` 方法发出一个带有十六进制字符串的对象`eth_chainId`，以及由Provider(提供者)确定的其他属性。
...
### message (信息)
Provider(提供者)发出`message`以向消费者传达任意消息。消息可能包括 JSON-RPC 通知、GraphQL 订阅和/或Provider(提供者)定义的任何其他事件。
```typescript copy
interface ProviderMessage {
  readonly type: string;
  readonly data: unknown;
...
## 附录 II：示例
```typescript copy
// 大多数Provider(提供者)在页面加载时都可以作为 window.ethereum 使用。
